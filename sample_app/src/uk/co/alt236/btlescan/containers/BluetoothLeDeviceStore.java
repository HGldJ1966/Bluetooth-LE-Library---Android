...
public class BluetoothLeDeviceStore {
    ...
    public void addDevice(BluetoothLeDevice device) {
        ...
        if (mDeviceMap.containsKey(key)) {
            mDeviceMap.get(key).updateRssiReading(device.getTimestamp(), device.getRssi());
        } else {
    ...
    public List<BluetoothLeDevice> getDeviceList() {
        final List<BluetoothLeDevice> methodResult = new ArrayList<BluetoothLeDevice>(
                mDeviceMap.values());
        ...
        Collections.sort(methodResult, new Comparator<BluetoothLeDevice>() {
            ...
            public int compare(BluetoothLeDevice arg0, BluetoothLeDevice arg1) {
                int compare = (int) (arg1.getRunningAverageRssi() - arg0.getRunningAverageRssi());
                if (compare == 0) {
                    compare = arg0.getId().compareToIgnoreCase(arg1.getId());
                }
                return compare;
            }
        });
    ...
    private String getListAsCsv() {
        ...
        for (BluetoothLeDevice device : list) {
            ...
            sb.append(CsvWriterHelper.addStuff(TimeFormatter.getIsoDateTime(device
                    .getFirstTimestamp())));
            ...
            sb.append(CsvWriterHelper.addStuff(ByteUtils.byteArrayToHexString(
                    device.getScanRecord(), true)));
    ...
    public void shareDataAsEmail(Context context) {
        ...
        try {
            ...
            final File outputFile = File.createTempFile("bluetooth_le_" + timeInMillis, ".csv",
                    outputDir);
            ...
            context.startActivity(Intent.createChooser(i,
                    context.getString(R.string.exporter_email_device_list_picker_text)));
